...
export default function Calendars({ initialDate = new Date(), NavbarComponent, onAddSchedule }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        let res;
        if (viewMode === "month") {
          res = await api.get('/calendar/month', { params: { year: y, month: m, type: typesParam } });
        } else if (viewMode === "week") {
          res = await api.get('/calendar/week', { params: { date, type: typesParam, dayStart: DAY_START } });
        } else {
          res = await api.get('/calendar/day',  { params: { date, type: typesParam, dayStart: DAY_START } });
        }
        setData(res.data);
      } catch (e) {
        setData(null);
  ...
  useEffect(() => {
    ...
    api.get('/schedules/stats/week', {
      params: {
        monday: monday.toISOString(),
        sunday: endOfDay(sunday).toISOString()
      }
    })
